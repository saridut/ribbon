...
import pytest
try:
    from ribbon import SpiralCircleInvolute
except:
...
def spiral():
    return SpiralCircleInvolute(1.0, 1.001)


@pytest.fixture(scope='module')
def spiral_data():
    fn = 'tests/test_data_spiral_circle_involute.txt'
    data = {}
...
def test_init_b():
    with pytest.raises(ValueError) as info:
        spiral = SpiralCircleInvolute(0, 1)
    assert info.type is ValueError


def test_init_r0():
    with pytest.raises(ValueError) as info:
        spiral = SpiralCircleInvolute(1, 1)
    assert info.type is ValueError
...
def test_get_tangent(spiral, spiral_data):
    ...
    assert np.allclose(tangent[:,0], spiral_data['tx'], 1e-7, 1e-14)
    assert np.allclose(tangent[:,1], spiral_data['ty'], 1e-7, 1e-14)
    #From theta
