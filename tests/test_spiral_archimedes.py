...
import pytest
try:
    from ribbon import SpiralArchimedes
except:
...
@pytest.fixture
def spiral():
    return SpiralArchimedes(1.0, 0.1)


@pytest.fixture
def spiral_data():
    fn = 'tests/test_data_spiral_archimedes.txt'
    data = {}
...
def test_init_b():
    with pytest.raises(ValueError) as info:
        spiral = SpiralArchimedes(0, 1)
    assert info.type is ValueError


def test_init_r0():
    with pytest.raises(ValueError) as info:
        spiral = SpiralArchimedes(1, -1)
    assert info.type is ValueError
